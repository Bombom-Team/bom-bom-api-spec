...
- If it is unclear whether the backend should receive individual query parameters or one `@ModelAttribute ...Request` DTO, ask the user before choosing between plain query parameters, a single `@query` model parameter, or the `x-spring-query-model` pattern.
